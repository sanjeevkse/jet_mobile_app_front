app.controller('boxDetailsSearchController', [
				'$scope', '$rootScope', '$state', 'toaster', 'frontEndData', 'apiFactory', 'formFactory', '$localStorage',  
function($scope, 	$rootScope, 	$state, toaster, frontEndData, apiFactory, formFactory, $localStorage){ 
	console.log('dfdf');
  ...
  $scope.openBoxStatusDate = function($event) {
	...
	var getBoxStatusForUpdaterNotTodays = function () {
		apiFactory
			.boxStatus
			.getBoxStatusSalesAgent({sales_agent_id: $localStorage.user.user.id})
			.then(function(retData) {
	...
	$scope.onChangeSearchInvoiceNumber = function() {
		apiFactory
			.boxStatus
			.getBoxStatusByInvoiceNumber({invoice_number: $scope.searchText})
			.then(function(retData) {
				$scope.arrNotToday = retData.data || [];
			}, function(retData) {
				$scope.arrNotToday = [];
