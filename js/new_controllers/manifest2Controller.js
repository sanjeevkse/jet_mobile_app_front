app.controller('manifest2Controller', [
...
function($scope, 	$rootScope, $state, toaster, frontEndData, apiFactory, Upload, appConstants, $state, blockUI, $timeout, $localStorage, formFactory, $location, $compile, $modal){ 
  ...
  $scope.openDestArrival = function($event) {
	...
	var initDataTable = function (argFilters) {
	    if($localStorage.user.user.user_role_id == 3) {
    		$scope.dataTab = $('#bt_table').DataTable({
                ...
                'columns': [
                    ...
                    { 
                        target: 1,
                        render: function ( data, type, row ) {
                            return "<a class=\"btn btn-warning btn-xs\" ui-sref=\"app.newBoxTracking({t: \'u\', id: '"+row.invoice_number+"'})\"><i class=\"fa fa-pencil\"></i></a>";
                        //  var href = $compile('<a ui-sref="app.newBoxTracking({t: \'u\', id: '+row.invoice_number+'})"></a>')($scope)[0].href;
                        //  return '<a class="btn btn-warning btn-xs" href="' + href + '"><i class="fa fa-pencil"></i></a>';
                       }
                    },
                    { 
                        target: 2,
	    ...
	    } else if($localStorage.user.user.user_role_id == 36) {
    		$scope.dataTab = $('#bt_table2').DataTable({
                "fixedHeader": true,
    			"destroy": true,
                "scrollX": true,
                scrollY: '560px',
    		...
    		$scope.dataTab = $('#bt_table3').DataTable({
                "fixedHeader": true,
    			"destroy": true,
                "scrollX": true,
                scrollY: '560px',
